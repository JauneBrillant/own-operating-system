        ...
        loop {
            ...
            match self.state {
                ...
                State::ScriptDataEndTagOpen => {
                    ...
                    self.reconsume = true;
                    self.state = State::ScriptData;
                    // According to the specification, both the '<' and '/' characters should be returned as separate tokens,
                    // But since next() can only return one token at a time, only the '<' token is returned.
                    return Some(HtmlToken::Char('<'));
                }
    ...
    fn test_attributes() {
        ...
        let expected = [
            HtmlToken::StartTag {
                ...
                self_closing: false,
                attributes: vec![
                    Attribute::generate_test_instance("class".to_string(), "A".to_string()),
                    Attribute::generate_test_instance("id".to_string(), "B".to_string()),
                    Attribute::generate_test_instance("foo".to_string(), "bar".to_string()),
                ],
            },
            HtmlToken::EndTag {
                tag: "p".to_string(),
